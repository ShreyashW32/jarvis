...
- Example (same segment): "I think the weather is great. What do you think, {name}?"
...
- Example (cross-segment reference):
  ...
  Current segment: "What do you think about that {name}?"
  → query is "what do you think about dinosaurs being cool" (resolve "that" from previous segment)
- Example: "{{name}}, how much does that iPhone cost?" after discussing iPhones
...
- The CURRENT segment may contain context + question (e.g., "The food was great. What do you think {name}?")
...
- Example: background segment says "I think dinosaurs are cool", then CURRENT says "What do you think about that {name}?" → resolve "that" to dinosaurs → query is "what do you think about dinosaurs being cool"

Output JSON only:
{{"directed": true/false, "query": "extracted query", "stop": true/false, "confidence": "high/medium/low", "reasoning": "brief explanation"}}

Examples:
- "{name} what time is it" → {{"directed": true, "query": "what time is it", "stop": false, "confidence": "high", "reasoning": "wake word with question"}}
- "The weather is lovely today. What do you think {name}?" → {{"directed": true, "query": "what do you think about the weather being lovely today", "stop": false, "confidence": "high", "reasoning": "wake word with contextual question"}}
- "I was looking at the new iPhone. {name} how much does it cost?" → {{"directed": true, "query": "how much does the new iPhone cost", "stop": false, "confidence": "high", "reasoning": "wake word with context from same utterance"}}
- Previous: "I made carbonara for dinner" + Current: "{name} can you find me a recipe for that" → {{"directed": true, "query": "find a recipe for carbonara", "stop": false, "confidence": "high", "reasoning": "resolved 'that' from previous segment about carbonara"}}
...
def create_intent_judge(cfg) -> Optional[IntentJudge]:
    """Create an intent judge from Luffy configuration.
    ...
    config = IntentJudgeConfig(
        assistant_name=str(getattr(cfg, "wake_word", "luffy")).capitalize(),
